...
public final class ClassFileUtil {
    ...
    public static String getClassName(ByteBuffer classFile, int cpEntryNumber, final int[] cpOffsets, StringBuilder scratch) {
        ...
        if (classFile.get(cpOffsets[cpEntryNumber]) != ClassFileUtil.CONSTANT_Class) {
    ...
    public static String getModuleName(ByteBuffer classFile, int cpEntryNumber, final int[] cpOffsets, String[] strCache, StringBuilder scratch) {
        ...
        if (classFile.get(cpOffsets[cpEntryNumber]) != ClassFileUtil.CONSTANT_Module) {
            throw new IllegalStateException("Invalid constant pool entry type");
        }
        int strEntry = classFile.getShort(cpOffsets[cpEntryNumber] + 1) & 0xffff;
        return getUtf8Entry(classFile, strEntry, cpOffsets, strCache, scratch);
    ...
    public static String getPackageName(ByteBuffer classFile, int cpEntryNumber, final int[] cpOffsets, String[] strCache, StringBuilder scratch) {
        ...
        if (classFile.get(cpOffsets[cpEntryNumber]) != ClassFileUtil.CONSTANT_Package) {
            throw new IllegalStateException("Invalid constant pool entry type");
        }
        int strEntry = classFile.getShort(cpOffsets[cpEntryNumber] + 1) & 0xffff;
        return getUtf8Entry(classFile, strEntry, cpOffsets, strCache, scratch);
    ...
    public static boolean stringEquals(final ByteBuffer classFile, final int cpEntryNumber, final int[] cpOffsets, final String str) {
        if (classFile.get(cpOffsets[cpEntryNumber]) != ClassFileUtil.CONSTANT_String) {
            throw new IllegalStateException("Invalid constant pool entry type");
        }
        int strEntry = classFile.getShort(cpOffsets[cpEntryNumber] + 1) & 0xffff;
        return utf8EntryEquals(classFile, cpOffsets[strEntry], str);

    ...
    final class Copier {
        ...
        static class Terminus implements NodeVisitor<Copier, Value, Node, BasicBlock, ValueHandle> {
            ...
            public ValueHandle visit(final Copier param, final AsmHandle node) {
                return param.getBlockBuilder().asm(node.getInstruction(), node.getConstraints(), node.getFlags(), node.getValueType());
            }
            ...
            public Value visit(final Copier param, final NewReferenceArray node) {
                param.copyNode(node.getDependency());
                return param.getBlockBuilder().newReferenceArray(node.getArrayType(), param.copyValue(node.getSize()));
            }
            ...
            public Value visit(final Copier param, final ProgramObjectLiteral node) {
                return node;
            }

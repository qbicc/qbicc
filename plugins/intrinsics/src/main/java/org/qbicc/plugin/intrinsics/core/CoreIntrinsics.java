...
public final class CoreIntrinsics {
    ...
    public static void registerJavaLangThreadIntrinsics(CompilationContext ctxt) {
        ...
        InstanceIntrinsic start0 = (builder, instance, target, arguments) -> {
            ...
            MethodDescriptor mdesc = MethodDescriptor.synthesize(classContext, voidPtrDesc, List.of(voidPtrDesc));
            ValueHandle mer = builder.staticMethod(vmHelpersDesc, "startTest", mdesc);

            // TODO call startTest to force a declare statement
            // declare i8* @startTest(i8*)
            //ValueType paramType = ctxt.getTypeSystem().getVoidType().getPointer();
            MethodElement me = ctxt.getVMHelperMethod("startTest");
            ValueType paramType = me.getParameters().get(0).getType();
            Value voidParam = ctxt.getLiteralFactory().zeroInitializerLiteralOfType(paramType);
            builder.call(mer, List.of(voidParam));
...
//            ValueHandle nativeThreadHandle = builder.instanceFieldOf(threadReference, nativeThreadField);
//            Value pthreadU64 = builder.valueConvert(pthreadNative, (SignedIntegerType)nativeThreadField.getType());
//            builder.store(nativeThreadHandle, pthreadU64, MemoryAtomicityMode.NONE);
            // TODO add thread to list for GC instead of storing this way
    ...
    static void registerOrgQbiccObjectModelIntrinsics(final CompilationContext ctxt) {
        ...
        // PThread.pthread_t_ptr get_threadNativeThread(Thread reference);
        FieldElement nativeThreadField = CoreClasses.get(ctxt).getThreadNativeThread();
        MethodDescriptor getNativeThreadDesc = MethodDescriptor.synthesize(classContext, pthreadPtrDesc, List.of(jltDesc));
        StaticIntrinsic getNativeThread = (builder, target, arguments) -> {
            Value pthreadSlot = builder.load(builder.instanceFieldOf(builder.referenceHandle(arguments.get(0)), nativeThreadField), MemoryAtomicityMode.NONE);
            PointerType returnType = (PointerType)target.getType().getReturnType();
            return builder.valueConvert(pthreadSlot, returnType);
        };
        intrinsics.registerIntrinsic(objModDesc, "get_threadNativeThread", getNativeThreadDesc, getNativeThread);
    }

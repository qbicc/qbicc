...
In native memory, introduce two new arrays indexed by `typeid`:

* `i8 qbicc_is_initialized[]`: Conceptually a single bit to indicated initialized or not.  If not initialized, then an upcall to the runtime
will handle the `{in-progress, failed}` states.
* `qbicc_clinit[]`: An array of function pointers to `<clinit>` methods to run for a given class.

These arrays need to be mutable so keep them separate from the rest of the `typeid`-related data as that data is constant.

The rest of the state can be managed on heap.  This can be done with array on the VMHelpers class that holds the
`Thread` while in progress, the `LC` lock, and the `Throwable` for failed initializations.
